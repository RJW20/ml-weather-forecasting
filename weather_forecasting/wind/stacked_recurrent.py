...
def stacked_recurrent() -> None:
    """Implements a stacked recurrent neural network for wind prediction.
    
    Saves the final model to models/wind/stacked_recurrent.keras
    Displays the loss curves for validation and training MAE, and saves
    the plot to figures/wind/stacked_recurrent_training.png.
    Displays a sample of the predictions against their targets for the test
    dataset, and saves the plot to
    figures/wind/stacked_recurrent_evaluation.png.
    ...
    save_location = "models/wind/stacked_recurrent.keras"
    train_model(model, train_dataset, val_dataset, save_location)
    plt.savefig(
        "figures/wind/stacked_recurrent_training.png",
        bbox_inches="tight",
    )
