...
def recurrent_dropout() -> None:
    """Implements a recurrent neural network with dropout for rainfall
    prediction.
    
    Saves the final model to models/rainfall/recurrent_dropout.keras
    Displays the loss curves for validation and training MAE, and saves
    the plot to figures/rainfall/recurrent_dropout_training.png.
    Displays a sample of the predictions against their targets for the test
    dataset, and saves the plot to
    figures/rainfall/recurrent_dropout_evaluation.png.
    ...
    save_location = "models/rainfall/recurrent_dropout.keras"
    train_model(model, train_dataset, val_dataset, save_location)
    plt.savefig(
        "figures/rainfall/recurrent_dropout_training.png",
        bbox_inches="tight",
    )
